...
int count = BoardDAO.getInstance().countBoard(boardName, option, search);
...
ArrayList<BoardBean> boardList = BoardDAO.getInstance().getBoardList(boardName, startRow, pageSize, option, search);
%>
<fieldset>
	<legend><%=boardName.toUpperCase() %></legend>
	<span>[전체 글개수 : <%=count %>] </span><br><br>
<table border="1"  id="table01">
<tr><th width="60px">글번호</th><th>제목</th><th width="140px">작성자</th><th width="100px">날짜</th><th width="40px">첨부</th><th width="60px">조회수</th></tr>
<%
...
<tr>
	...
	<td><a href="content.jsp?boardName=<%=boardName %>&num=<%=bb.getIdx()%>" id="contentLink"> <%=bb.getSubject()%></a></td>
	<td id="data"><%=bb.getName()%></td>
	<td id="data"><%=sdf.format(bb.getDateTime())%></td>
	<td id="data"><%=bb.getReadCount()%></td>
	<td id="data"><%if (bb.getFile()==null) {out.print("");} else {%><img src="attachmentIcon.png" alt="첨부파일" width="10px" height="10px"><%}%></td>
</tr>
<%
}
%>
...
<% if (id != null) {
	...
	%>
	<form action="listSearch.jsp?boardName=<%=boardName %>&search=<%=search %>&pageNum=1" method="post"><select name="option"><option value="subject" selected>제목</option><option value="content">내용</option><option value="name">작성자</option></select><input type="text" name="search"> <input type="image" src="search.png" width="15px" height="15px"></form>
</fieldset>
<p style="float: right">
<%
